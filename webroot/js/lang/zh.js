...
I18N.register('zh', {
  ...
  info_androidid_msg: '让此设备配置下的应用读取伪造的 ANDROID_ID（来自 Settings.Secure 的标识符）。16 位十六进制字符。无需重启即可应用，且模块在应用运行前卸载，因此应用内存中不留任何 COPG 痕迹。留空则保留真实 ID。注意：执意的应用仍可通过其他方式读取真实 ID——此项仅改变标准 Settings 查询。',
